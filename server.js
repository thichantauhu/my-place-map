...
async function gogodukRequest(pathname, params) {
  if (!GOGODUK_API_KEY) {
    throw Object.assign(new Error('GOGODUK_API_KEY_NOT_CONFIGURED'), { code: 'GOGODUK_API_KEY_NOT_CONFIGURED' });
  }
  ...
  if (!response.ok) {
    throw Object.assign(new Error(data?.message || data?.error || `GoGoDuk HTTP ${response.status}`), {
...
async function addressSuggest(res, query, sessionToken) {
  try {
    const data = await gogodukRequest('/v1/suggest', {
      input: query,
      lang: 'vi',
      country: 'VN',
      sessionToken
    ...
    return send(res, error.status || 503, {
      error: error.code || 'GOGODUK_ERROR',
      message: error.message || 'Không thể tìm địa chỉ bằng GoGoDuk.'
    });
  }
}
...
async function placeResolve(res, id, sessionToken) {
  try {
    const data = await gogodukRequest('/v1/place/resolve', {
      id,
      lang: 'vi',
      sessionToken
    });
    ...
    return send(res, error.status || 503, {
      error: error.code || 'GOGODUK_ERROR',
      message: error.message || 'Không thể lấy tọa độ địa chỉ.'
    });
  }
}

async function handleApi(req, res, url) {
  if (req.method === 'GET' && url.pathname === '/api/address-search') {
    ...
    if (!GOGODUK_API_KEY) return send(res, 503, {
      error: 'GOGODUK_API_KEY_NOT_CONFIGURED',
      message: 'Chưa cấu hình GoGoDuk API key.'
    });
    return addressSuggest(res, q, sessionToken);
  }

  if (req.method === 'GET' && url.pathname === '/api/address-resolve') {
