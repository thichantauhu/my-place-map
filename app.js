...
function openAddDialog(lat=null,lng=null){els.error.textContent='';els.addressResults.innerHTML='';els.name.value='';els.address.value='';els.note.value='';els.coord.value='';els.lat.value='';els.lng.value='';els.selectedLocation.textContent='Chưa chọn vị trí chính xác';els.want.checked=false;els.category.value='food';if(validCoord(lat,lng))syncCoords(lat,lng);els.dialog.showModal();setTimeout(()=>els.name.focus(),50)}
...
async function savePlace(e){e.preventDefault();els.error.textContent='';const name=els.name.value.trim(),c=readCoords();if(!name){els.error.textContent='Hãy nhập tên địa điểm.';return}if(!c){els.error.textContent='Hãy dán tọa độ theo dạng: Vĩ độ, Kinh độ (ví dụ 10.800829838769747, 106.68482208597538).';return}const address=els.address.value.trim();const duplicate=places.some(p=>p.name.trim().toLowerCase()===name.toLowerCase()&&Math.abs(Number(p.lat)-c.lat)<1e-12&&Math.abs(Number(p.lng)-c.lng)<1e-12);if(duplicate){els.error.textContent='Địa điểm này đã tồn tại.';return}const place=normalizePlace({id:window.crypto?.randomUUID?.()||`${Date.now()}-${Math.random()}`,name,address,category:els.category.value,note:els.note.value.trim(),lat:c.lat,lng:c.lng,want:els.want.checked,rating:null,createdAt:Date.now()});els.error.textContent='⏳ Đang lưu...';try{await api('/api/places',{method:'POST',body:JSON.stringify(place)});places.push(place);savePlaces();els.radius.value='999';closePlaceDialog();render();map.setView([c.lat,c.lng],18,{animate:true});els.mapHint.textContent='📍 Đã lưu địa điểm tại đúng tọa độ Google Maps.'}catch(err){els.error.textContent='Không lưu được địa điểm vào dữ liệu chung. Thử lại sau.'}}
